 ...
 * 
 * @param memoryRequirements The required size of the state memory.
...
TAPI b8 InputIsKeyUp(keys key);
TAPI b8 InputWasKeyDown(keys key);
TAPI b8 InputWasKeyUp(keys key);

void InputProcessKey(keys key, b8 pressed);

// mouse input
TAPI b8 InputIsButtonDown(buttons button);
TAPI b8 InputIsButtonUp(buttons button);
TAPI b8 InputWasButtonDown(buttons button);
TAPI b8 InputWasButtonUp(buttons button);
TAPI void InputGetMousePosition(s32* x, s32* y);
TAPI void InputGetPreviousMousePosition(s32* x, s32* y);
